...
export const DynamicPhotoCard: React.FC<DynamicPhotoCardProps> = ({
  ...
  const updateImage = useWorkflowStoreTwoExtraThree((state) => state.updateImage);
  const removeImage = useWorkflowStoreTwoExtraThree((state) => state.removeImage);
  ...
  const handleTakePhoto = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      const filename = `loading_${image.uuid}_${Date.now()}.jpg`;
  ...
  return (
    <View style={styles.card}>
      <View style={styles.header}>
        <Text style={styles.title}>Foto #{index} - Proceso de Carga</Text>
      ...
      <View style={styles.commentContainer}>
        ...
        <TextInput
          ...
          placeholder="Describe lo que se observa en esta foto del proceso..."
