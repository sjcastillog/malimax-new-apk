...
export const ValidationItemSection: React.FC<ValidationItemSectionProps> = ({
  ...
  const referencePhoto = useWorkflowStoreThreeExtraThree(
    (state) => (state as any)[photoIdKey],
  );
  const validationPhoto = useWorkflowStoreThreeExtraThree(
    (state) => (state as any)[validationPhotoKey],
  );
  const status = useWorkflowStoreThreeExtraThree(
    (state) => (state as any)[statusKey],
  );
  const comment = useWorkflowStoreThreeExtraThree(
    (state) => (state as any)[commentKey],
  );

  // Obtener setters del store
  const setValidationPhoto = useWorkflowStoreThreeExtraThree((state) => {
    const setterName = `set${validationPhotoKey.charAt(0).toUpperCase()}${validationPhotoKey.slice(1)}`;
    return (state as any)[setterName];
  });

  const setStatus = useWorkflowStoreThreeExtraThree((state) => {
    const setterName = `set${statusKey.charAt(0).toUpperCase()}${statusKey.slice(1)}`;
    return (state as any)[setterName];
  });

  const setComment = useWorkflowStoreThreeExtraThree((state) => {
