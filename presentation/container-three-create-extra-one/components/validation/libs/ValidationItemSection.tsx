...
export const ValidationItemSection: React.FC<ValidationItemSectionProps> = ({
  ...
  const referencePhoto = useWorkflowStoreThreeExtraOne(
    (state) => (state as any)[photoIdKey],
  );
  const validationPhoto = useWorkflowStoreThreeExtraOne(
    (state) => (state as any)[validationPhotoKey],
  );
  const status = useWorkflowStoreThreeExtraOne((state) => (state as any)[statusKey]);
  const comment = useWorkflowStoreThreeExtraOne(
    (state) => (state as any)[commentKey],
  );

  // Obtener setters del store
  const setValidationPhoto = useWorkflowStoreThreeExtraOne((state) => {
    const setterName = `set${validationPhotoKey.charAt(0).toUpperCase()}${validationPhotoKey.slice(1)}`;
    return (state as any)[setterName];
  });

  const setStatus = useWorkflowStoreThreeExtraOne((state) => {
    const setterName = `set${statusKey.charAt(0).toUpperCase()}${statusKey.slice(1)}`;
    return (state as any)[setterName];
  });

  const setComment = useWorkflowStoreThreeExtraOne((state) => {
