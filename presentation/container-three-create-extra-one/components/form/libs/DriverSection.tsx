...
export const DriverSection = () => {
  const driverName = useWorkflowStoreThreeExtraOne((state) => state.driverName);
  const setDriverName = useWorkflowStoreThreeExtraOne(
    (state) => state.setDriverName,
  );
  const driverIdentification = useWorkflowStoreThreeExtraOne(
    (state) => state.driverIdentification,
  );
  const setDriverIdentification = useWorkflowStoreThreeExtraOne(
    (state) => state.setDriverIdentification,
  );
  const plateVehicle = useWorkflowStoreThreeExtraOne(
    (state) => state.plateVehicle,
  );
  const setPlateVehicle = useWorkflowStoreThreeExtraOne(
    (state) => state.setPlateVehicle,
  );

  const companyTransport = useWorkflowStoreThreeExtraOne(
    (state) => state.companyTransport,
  );
  const setCompanyTransport = useWorkflowStoreThreeExtraOne(
