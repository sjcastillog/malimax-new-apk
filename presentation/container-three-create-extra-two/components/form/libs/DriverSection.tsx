...
export const DriverSection = () => {
  const driverName = useWorkflowStoreThreeExtraTwo((state) => state.driverName);
  const setDriverName = useWorkflowStoreThreeExtraTwo(
    (state) => state.setDriverName,
  );
  const driverIdentification = useWorkflowStoreThreeExtraTwo(
    (state) => state.driverIdentification,
  );
  const setDriverIdentification = useWorkflowStoreThreeExtraTwo(
    (state) => state.setDriverIdentification,
  );
  const plateVehicle = useWorkflowStoreThreeExtraTwo(
    (state) => state.plateVehicle,
  );
  const setPlateVehicle = useWorkflowStoreThreeExtraTwo(
    (state) => state.setPlateVehicle,
  );

  const companyTransport = useWorkflowStoreThreeExtraTwo(
    (state) => state.companyTransport,
  );
  const setCompanyTransport = useWorkflowStoreThreeExtraTwo(
