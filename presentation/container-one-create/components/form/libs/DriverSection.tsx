...
export const DriverSection = () => {
  const driverName = useWorkflowStoreOneZero((state) => state.driverName);
  const setDriverName = useWorkflowStoreOneZero((state) => state.setDriverName);
  const driverIdentification = useWorkflowStoreOneZero(
    (state) => state.driverIdentification,
  );
  const setDriverIdentification = useWorkflowStoreOneZero(
    (state) => state.setDriverIdentification,
  );
  const plateVehicle = useWorkflowStoreOneZero((state) => state.plateVehicle);
  const setPlateVehicle = useWorkflowStoreOneZero(
    (state) => state.setPlateVehicle,
  );
  const companyTransport = useWorkflowStoreOneZero(
    (state) => state.companyTransport,
  );
  const setCompanyTransport = useWorkflowStoreOneZero(
