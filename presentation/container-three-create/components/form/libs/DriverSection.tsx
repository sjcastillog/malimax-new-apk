...
export const DriverSection = () => {
  const driverName = useWorkflowStoreThreeZero((state) => state.driverName);
  const setDriverName = useWorkflowStoreThreeZero(
    (state) => state.setDriverName,
  );
  const driverIdentification = useWorkflowStoreThreeZero(
    (state) => state.driverIdentification,
  );
  const setDriverIdentification = useWorkflowStoreThreeZero(
    (state) => state.setDriverIdentification,
  );
  const plateVehicle = useWorkflowStoreThreeZero((state) => state.plateVehicle);
  const setPlateVehicle = useWorkflowStoreThreeZero(
    (state) => state.setPlateVehicle,
  );

  const companyTransport = useWorkflowStoreThreeZero(
    (state) => state.companyTransport,
  );
  const setCompanyTransport = useWorkflowStoreThreeZero(
