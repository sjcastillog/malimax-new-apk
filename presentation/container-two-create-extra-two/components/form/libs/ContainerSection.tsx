  ...
  return (
    <>
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>📦 DATOS DEL PROCESO 1</Text>
...
const ContainerSearchField = () => {
  ...
  const setHourInit = useWorkflowStoreTwoExtraTwo((state) => state.setHourInit);
  const setContainer = useWorkflowStoreTwoExtraTwo(
    (state) => state.setContainer,
  );
  const setClient = useWorkflowStoreTwoExtraTwo((state) => state.setClient);
  const setClientId = useWorkflowStoreTwoExtraTwo((state) => state.setClientId);
  const setClientIdentification = useWorkflowStoreTwoExtraTwo(
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Buscar datos del proceso 1
      const data = await getWorkflowOneByContainerForNextProcess(container);

      if (!data) {
        Alert.alert(
          "No Encontrado",
          `No se encontró el contenedor "${container}" en el proceso 1.\n\n¿Deseas continuar de todas formas?`,
          [
...
const ClientInfo = () => {
  ...
  const clientIdentification = useWorkflowStoreTwoExtraTwo(
