...
export const ValidationItemSection: React.FC<ValidationItemSectionProps> = ({
  ...
  const referencePhoto = useWorkflowStoreTwoExtraOne(
    (state) => (state as any)[photoIdKey],
  );
  const validationPhoto = useWorkflowStoreTwoExtraOne(
    (state) => (state as any)[validationPhotoKey],
  );
  const status = useWorkflowStoreTwoExtraOne(
