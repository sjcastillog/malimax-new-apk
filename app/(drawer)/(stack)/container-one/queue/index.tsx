...
export default function OneQueueScreen() {
  ...
  const loadQueue = async () => {
    try {
      const items = await workflowDB.getAllQueue("one");
      setQueue(items);

      const queueStats = await workflowDB.getQueueCount("one");
      setStats(queueStats);
    } catch (error) {
      ...
      Alert.alert(
        ...
        [
          { text: "Cancelar", style: "cancel" },
          {
            ...
            onPress: async () => {
              setSyncing(true);
              const result = await QueueSyncService.sendAllPending("one");
              setSyncing(false);
  ...
  const handleSendItem = async (id: string) => {
    try {
      ...
      setSendingItem(id);
      const success = await QueueSyncService.sendItem(id, "one");
      setSendingItem(null);
