...
export default function TwoQueueScreen() {
  ...
  const loadQueue = async () => {
    try {
      const items = await workflowDB.getAllQueue("two");
      setQueue(items);

      const queueStats = await workflowDB.getQueueCount("two");
      setStats(queueStats);
    } catch (error) {
      ...
      Alert.alert(
        ...
        [
          { text: "Cancelar", style: "cancel" },
          {
            ...
            onPress: async () => {
              setSyncing(true);
              const result = await QueueSyncService.sendAllPending("two");
              setSyncing(false);
  ...
  const handleSendItem = async (id: string) => {
    try {
      ...
      setSendingItem(id);
      const success = await QueueSyncService.sendItem(id, "two");
      setSendingItem(null);
